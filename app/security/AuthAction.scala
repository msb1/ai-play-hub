...
import repositories.UserRepository.getClass

import scala.concurrent.{ExecutionContext, Future}
import scala.util.{Failure, Success}

// A custom request type to hold our JWT claims, we can pass these on to the handling action
case class UserRequest[A](claims: Claims, token: String, request: Request[A]) extends WrappedRequest[A](request)
...
class AuthAction @Inject()(bodyParser: BodyParsers.Default)(implicit ec: ExecutionContext)
  ...
  // Called when a request is invoked. Validate the bearer token here
  // Allow the request to proceed if it is valid.
